...
결과는 `dist/site/`에 생성됩니다. 사이트에는 작품별 패치 ZIP을 포함하지 않습니다. 방문자는 항상 자신의 패치 ZIP과 원본 HDM을 차례로 선택합니다.
...
방문자는 브라우저 파일 선택기로 로컬 패치 ZIP과 원본 HDM을 차례로 엽니다. 원본 검사, 파일별 BPS 적용, FAT12 재조립과 결과 검증은 브라우저 메모리에서 실행되며 결과 HDM은 로컬 다운로드로만 만들어집니다.

패처에는 패치 ZIP·원본 HDM·결과 HDM을 받는 서버 API, 요청 본문 전송, 계정이나 서버 저장소가 없습니다. 방문자 브라우저의 네트워크 요청은 HTML·JavaScript·WebAssembly를 받는 정적 `GET`뿐입니다. 따라서 정적 호스트의 접근 로그에도 사용자가 고른 로컬 파일의 이름이나 내용이 전달되지 않습니다.

S3에는 `dist/site/`의 정적 자산만 올라가야 합니다. 패치 ZIP, 원본 HDM, 결과 HDM, 저장 데이터와 내부 검수 캡처는 업로드하지 않습니다.
