...
이 문서는 패처 운영자가 서버 코드 없이 정적 사이트를 빌드해 Amazon S3와 CloudFront에 배포하는 절차를 설명합니다. 배포 대상은 `dist/site/`뿐이며, 원본·결과·작성용 매체 이미지는 포함하지 않습니다.
...
결과는 `dist/site/`에 생성됩니다. 사이트에는 작품별 패치 ZIP을 포함하지 않습니다. 방문자는 항상 자신의 패치 ZIP과 원본 이미지를 차례로 선택합니다.
...
- 첫 화면에서 별도 설정 요청 없이 패치 ZIP 입력을 바로 사용할 수 있는가
- 패치 ZIP을 고르면 패치 제목과 지원 원본 조건을 읽는가
- 원본 이미지에 적용한 결과가 레시피의 출력 SHA-256과 일치하는가
- 내려받은 결과 이미지의 파일명과 크기가 레시피와 일치하는가
...
`index.rsc`, `404.html`과 그 밖의 루트 파일은 첫 번째 `sync`에서 `no-cache`로 올라갑니다. AWS CLI나 업로드 도구가 기존 객체의 메타데이터를 유지했다면 S3 콘솔에서 `index.html`과 WebAssembly의 `Content-Type`과 `Cache-Control`을 다시 확인합니다.
...
aws cloudfront create-invalidation \
  ...
  --paths '/' '/index.html' '/404.html' '/index.rsc'
```
...
개발자 도구의 네트워크 화면에서는 WebAssembly 응답이 `application/wasm`인지 확인합니다.

## 사용자가 받는 동작

방문자는 브라우저 파일 선택기로 로컬 패치 ZIP과 원본 이미지를 차례로 엽니다. 원본 검사, 파일별 BPS 적용, 형식별 재조립과 결과 검증은 브라우저 메모리에서 실행되며 결과 이미지는 로컬 다운로드로만 만들어집니다.

패처에는 패치 ZIP·원본 이미지·결과 이미지를 받는 서버 API, 요청 본문 전송, 계정이나 서버 저장소가 없습니다. 방문자 브라우저의 네트워크 요청은 HTML·JavaScript·WebAssembly를 받는 정적 `GET`뿐입니다. 따라서 정적 호스트의 접근 로그에도 사용자가 고른 로컬 파일의 이름이나 내용이 전달되지 않습니다.

S3에는 `dist/site/`의 정적 자산만 올라가야 합니다. 패치 ZIP, 원본 이미지, 결과 이미지, 저장 데이터와 내부 검수 캡처는 업로드하지 않습니다.
