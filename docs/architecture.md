...
├─ app/components/multi-file-picker.tsx 다중 로컬 이미지 입력
...
├─ iso9660.rs                            raw Mode1 추출·논리 ISO 조립
├─ iso_directory_package.rs              ISO 디렉터리 패키지 제작·적용
...
`pipeline.rs`는 FAT12 파일 수집, 변환과 HDM 조립 순서만 조정합니다. ISO 경로는 `iso_directory_package.rs`가 원본 디렉터리 추출과 논리 ISO 조립을 조정합니다. ZIP 공통 처리는 `patch_package.rs`, BPS 자체는 공개 유틸리티, FAT12 소비는 `fat12.rs`가 맡습니다. `fat12_legacy.rs`는 최초 공개된 ASCII 패키지의 목표 바이트를 재현할 때만 호출됩니다. 테스트는 구현 파일 안에 넣지 않고 모두 `_tests.rs`에 둡니다.
...
ISO9660 디렉터리 형식은 정확한 raw Mode1 원본 이미지와 제작자 로컬 content 디렉터리를 입력으로 받습니다. 원본의 지정 디렉터리에서 검증한 파일명 집합만 content와 대응하며, 결과는 고정 레이아웃의 논리 ISO로 새로 조립합니다. 원본 CD의 다른 디렉터리·트랙·섹터를 결과에 복사하지 않습니다.
...
현재 패치 세트의 각 내포 ZIP은 기존 FAT12 단일 패키지 검사를 통과해야 하고, 적용도 같은 FAT12 적용 함수를 호출합니다. 상위 계층은 내부 형식을 복제하지 않으며 패키지 바이트 해시, 필수 구성원 집합과 원본·결과 정체의 비모호성만 소유합니다. ISO9660 디렉터리 형식은 현재 단일 패키지만 지원합니다.

서버에서 받는 것은 정적 HTML·JavaScript·WebAssembly뿐입니다. 사용자가 고른 패치 ZIP과 원본 이미지, 조립 결과는 브라우저의 `File`, `Uint8Array`, `Blob`에만 존재하며 서버 API나 업로드 경로가 없습니다.
