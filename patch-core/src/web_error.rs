use crate::patch_set::UnsupportedPatchSetFormat;
...
const UNSUPPORTED_ARTIFACT_MESSAGE: &str =
    "이 ZIP은 현재 지원하는 PC-98 FAT12 패치 형식이 아닙니다. 이 패처용 ZIP을 선택하세요.";
...
pub(crate) fn describe_artifact_selection_failure(error: anyhow::Error) -> String {
    if error.chain().any(|cause| {
        cause.downcast_ref::<UnsupportedPackageFormat>().is_some()
            || cause.downcast_ref::<UnsupportedPatchSetFormat>().is_some()
    }) {
        UNSUPPORTED_ARTIFACT_MESSAGE.to_owned()
    } else {
        INVALID_PACKAGE_MESSAGE.to_owned()
    }
}
